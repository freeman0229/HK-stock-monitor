    ...
    def fetch(self, stock_code: str, d: date) -> dict | None:
        ...
        for attempt in range(1, 4):
            try:
                ...
                code_field.fill(code5)

                human_sleep(PRE_SLEEP_MIN, PRE_SLEEP_MAX)

                # ── Click 搜尋 and wait for ASP.NET postback ──────────────────
                # Use click then wait_for_load_state — avoids expect_navigation
                # race condition (listener must be set up BEFORE the click fires,
                # but the click itself may be instant). wait_for_load_state is
                # called immediately after click and waits for the postback to settle.
                ...
                if any(p in content for p in BLOCK_PATTERNS):
                    ...
                    if attempt < 3:
                        human_sleep(90, 180)
                        self._new_context()   # fresh context + cookies after block
                        continue
                    self._on_sdw = False   # page is in blocked state — force re-nav next call
                    return None
                ...
                if attempt < 3:
                    wait = attempt * 15
                    log.warning("fetch (%s %s) attempt %d failed: %s — retry in %ds",
